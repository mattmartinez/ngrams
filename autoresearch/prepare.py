...
def _save_tokenizer_config(path, pattern, mergeable_ranks, special_tokens):
    """Serialize tokenizer config to JSON (safe, no pickle)."""
    payload = {
        "name": "rustbpe",
        "pat_str": pattern,
        "mergeable_ranks": [
...
def _load_tokenizer_config(path):
    """Deserialize tokenizer config from JSON."""
    with open(path, "r", encoding="utf-8") as f:
        payload = json.load(f)
    mergeable_ranks = {
        base64.b64decode(token_b64.encode("ascii")): int(rank)
        for token_b64, rank in payload["mergeable_ranks"]
    }
...
def train_tokenizer():
    ...
    # Save tokenizer config as JSON (no pickle — prevents cache poisoning RCE)
    _save_tokenizer_config(tokenizer_json, pattern, mergeable_ranks, special_tokens)
...
class Tokenizer:
    ...
    def from_directory(cls, tokenizer_dir=TOKENIZER_DIR):
        tokenizer_json = os.path.join(tokenizer_dir, "tokenizer.json")
        legacy_pickle = os.path.join(tokenizer_dir, "tokenizer.pkl")
        if os.path.exists(tokenizer_json):
            enc = _load_tokenizer_config(tokenizer_json)
            ...
            raise FileNotFoundError(f"Tokenizer config not found: {tokenizer_json}")
        return cls(enc)
