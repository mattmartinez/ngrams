...
# Verify CUDA is available before proceeding (PR #176)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        softcap = 15.0  # must be float for bfloat16 division
...
# Print GPU info for debugging and verification (PR #177)
...
# Dynamic GPU detection for accurate MFU (PR #12)
...
while True:
    ...
    t0 = time.time()
    train_loss = 0.0  # accumulate loss properly across micro-steps (PR #70)
    ...
    if math.isnan(train_loss_f) or train_loss_f > 100:
        print(f"FAIL: Loss {'is NaN' if math.isnan(train_loss_f) else 'exploded'} at step {step} (loss={train_loss_f})")
        exit(1)
...
# Write structured results for reliable agent consumption (PR #79)
